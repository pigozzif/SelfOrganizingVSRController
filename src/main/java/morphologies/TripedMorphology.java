...
public class TripedMorphology extends Morphology {

    private final int legWidth;

    public TripedMorphology(int w, int h, String sensorConfig, int l) {
    ...
    public void fillBody() {
        ...
        int mid = Math.floorDiv(this.body.getW(), 2);
        for (int j=0; j < this.body.getH(); ++j) {
            if (j < this.legWidth) {
                this.body.set(mid, j, SerializationUtils.clone(sensingVoxel));
            }
        }*/
    }
