...
public class Utils {
    ...
    public static Accumulator.Factory<Event<?, ? extends Robot<?>, ? extends Outcome>, File> bestVideo(double transientTime, double episodeTime, Settings settings) {
        return Accumulator.Factory.<Event<?, ? extends Robot<?>, ? extends Outcome>>last().then(
                event -> {
                    ...
                    SortedMap<Long, String> terrainSequence = Main.getSequence((String) event.getAttributes().get("terrain"));
                    SortedMap<Long, String> transformationSequence = Main.getSequence((String) event.getAttributes().get("transformation"));
    ...
    public static Function<Event<?, ? extends Robot<?>, ? extends Outcome>, Collection<Main.ValidationOutcome>> validation(
        ...
        return event -> {
            Robot<?> robot = SerializationUtils.clone(Misc.first(event.getOrderedPopulation().firsts()).getSolution());
            List<Main.ValidationOutcome> validationOutcomes = new ArrayList<>();
            for (String validationTerrainName : validationTerrainNames) {
                for (String validationTransformationName : validationTransformationNames) {
                    for (int seed : seeds) {
                        ...
                        Function<Robot<?>, Outcome> validationLocomotion = Main.buildLocomotionTask(
                                validationTerrainName,
                                episodeTime,
                                random
                        );
                        Outcome outcome = validationLocomotion.apply(robot);
                        validationOutcomes.add(new Main.ValidationOutcome(
                                event,
                                Map.ofEntries(
